...
describe("Workflow end to end", () => {
  it("runs a real script whose agents reach real sessions with the script's prompts", async () => {
    ...
    const run = await runPrintMode({
      ...
      respond: context => {
        const isParent = sessionToolNames(context).includes("SubagentWorkflow");
        if (!isParent) {
          childPrompts.push(asText(context));
          childSystemPrompts.push(transcriptSystemPrompt(context));
    ...
    const run = await runPrintMode({
      ...
      respond: context => {
        ...
        if (!isParent) {
          ...
          const alreadyAnswered = sessionToolNames(context).length > 0 && /Recorded\./.test(seen);
