...
- **`/resume-filtered`** — pi's `/resume` picker can't be filtered from an extension (it renders every session, including hundreds of `general-purpose#…` subagent runs). This parallel command lists the same store minus every session spawned by another session, and switches to your pick; `/resume-filtered all` covers all folders like core's Tab scope
