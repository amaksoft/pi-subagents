...
- **Claude Code look & feel** — same tool names, calling conventions, and UI patterns (`Agent`, `get_subagent_result`, `steer_subagent`, `stop_subagent`) — feels native
...
- **Per-agent `timeout`** — wall-clock budget in minutes from run start (queued time is free) on `Agent` (nested included). Expires through the normal stop path with partial output preserved and the cause named; extend a live budget with `snooze_subagent`. Omit for unlimited
- **`/resume-filtered`** — pi's `/resume` picker always renders its whole tree expanded (and the generic extension select dialog has no windowing at all, so it breaks at our session counts). This parallel command keeps core's parent/child relations but loads collapsed: roots only, each badged with its hidden child count (`· 8 child sessions (→ to expand)`). `↑↓` navigate · `→` expand · `←` collapse · `Space` toggle · `Enter` resume · type to filter · windowed scrolling for long lists; `/resume-filtered all` covers all folders like core's Tab scope
...
- **Event bus** — lifecycle events (`subagents:created`, `started`, `completed`, `failed`, `steered`, `stopped`, `compacted`) emitted via `pi.events`, enabling other extensions to react to sub-agent activity
...
Running [workflows](#subagentworkflow) appear as a single `workflow` row above the agents, carrying their agent counts in place of a description. `Enter` on one opens the same two-pane inspector `/agents → Workflows` does, rather than a conversation overlay. A run's own agents are *not* listed separately — they belong to the run, which reports for them, so they are filtered out of the fleet list, the above-editor widget, the `/agents` menus and `@handle` resolution exactly as nested children are. They are also outside the `maxConcurrent` pool: the run has its own concurrency cap, and routing a fan-out through the session pool as well would let one workflow starve everything else. The agents are ordered earliest-launched first. Running agents open their live conversation on `Enter`; queued and session-less rows cannot open yet but still show — `x` (twice) stops them from the list without opening. At an **empty prompt**, press `↓` (or `←`) to move focus from the prompt into the list — the selected row is marked `●`, the rest `○`. The selected row renders in the theme's primary text color rather than the muted/dim treatment of the others; an agent with a configured `color` shows its badge there too, bolded. `↑`/`↓` move the selection, `Enter` opens the selected agent's live conversation overlay (it auto-updates as the agent works), `x` (twice to confirm) stops the selected running or queued agent without opening it, and `Esc` (or `↑` above `main`) returns to the prompt. Selecting `main` returns to the normal view. Inside the overlay, press `Enter` to steer the running agent — type a message and `Enter` to send it (`Esc` or an empty submit returns), and it redirects the agent the same way the `steer_subagent` tool does. A viewer stays open when its agent finishes so you can read the final output, and finished agents linger in the list for a few seconds before dropping out. Typing anything at a non-empty prompt behaves normally — the list only captures keys when the prompt is empty. Disable it entirely via `/agents → Settings → Fleet view`.
...
| `session_dir` | pi default | Optional session directory when `persist_session: true`; omitted files the session under `.subagents/` of pi's normal session location — core's pickers read one directory level, so subagent sessions stay out of `/resume` while `@handle` resume (absolute paths) keeps working. Relative paths resolve from the agent cwd. An explicit override (or `isolation: worktree`) lists separately instead, showing as a root |
| `allowed_subagents` | none | Opt in to scoped nested `Agent`, `get_subagent_result`, `steer_subagent`, and `stop_subagent` tools. Omitted / empty / `none` / `false` = no nesting; `all` (or `"*"` / `true`) = any enabled agent; comma-separated list = only those agent types |
...
| `name` | string | no | Memorable name for this agent (`auth-audit`), addressable as `@name` and accepted by `steer_subagent`/`stop_subagent`/`get_subagent_result`. Additive — the type-derived handle is still assigned |
...
Stop a running or queued top-level agent — the kill switch for stuck, looping, or no-longer-needed agents that steering cannot redirect. The agent aborts immediately and a `subagents:stopped` event fires; check FleetView or `get_subagent_result` for status. A run that started keeps partial output — read it with `get_subagent_result`; a stopped run can be resumed or re-spawned. Nested children belong to their owner — this tool refuses them.
...
**Stall auto-abort** (`stallAutoAbort`, default `false`): whether running agents silent past the stall threshold are aborted automatically. The threshold itself is `stallThresholdMs` in `subagents.json` (default 10 minutes, clamped to 1 minute–24 hours on load; json-only, no menu row). Silence means no tool output, no streamed text, and no usage — the heartbeat in `status-note.ts`. Queued agents are never affected (their silence is waiting, not wedging). When it fires, the abort flows through the normal stop path: STOPPED note plus the regular completion notification, right after the stall nudge, so the parent always learns why the run ended. The threshold is `stallThresholdMs` in `subagents.json` (default 10 minutes, clamped to 1 minute–24 hours on load). Toggle via `/agents → Settings → Stall auto-abort`; applied live. With it off (the default), stalls are only flagged — in FleetView, `get_subagent_result`, and a one-per-episode `subagents:stalled` notification — and stopping stays a human/model decision via `stop_subagent`.
...
| `subagents:stopped` | Agent stopped — tool, FleetView/`/agents` key, RPC, or abortAll; queued stops included (they never reach `completed`/`failed`) | identical payload to `subagents:completed`. Running stops additionally surface as `subagents:failed` at settle, same payload with `status: "stopped"` |
...
The five agent-lifecycle events — `subagents:started`, `:completed`, `:failed`, `:stopped`, `:compacted` — are emitted for **top-level agents only**. Nested subagents and a workflow's children emit nothing at all; they report through the parent or workflow that owns them.
